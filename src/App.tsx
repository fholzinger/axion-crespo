...
import { 
  ...
  BarChart3, Database, CircleDollarSign, Printer, AlertTriangle, Download
} from 'lucide-react';
...
const RRHHView = () => {
  ...
  if (!sesionActiva) return (
    <div className="bg-white p-8 rounded-[2rem] shadow-xl border-t-8 border-blue-600">
      ...
      {empleadoSel && <div className="space-y-4">
        <input type="password" value={pinInput} onChange={(e) => setPinInput(e.target.value)} placeholder="PIN" className="w-full p-4 text-center text-3xl border-2 rounded-2xl font-black outline-none" />
        <button onClick={() => {
        ...
        }} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black shadow-lg uppercase">Ingresar</button>
      </div>}
    </div>
  );
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      <div className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border">
        <p className="font-black text-blue-900 text-xs uppercase">{sesionActiva}</p>
        <button onClick={() => setSesionActiva(null)} className="text-red-500 font-bold text-xs uppercase">Salir</button>
      </div>
      {!subPantalla ? (
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-xl relative">
          ...
          {subPantalla === 'vacaciones' && <div className="space-y-4">
            ...
            <button onClick={() => enviar("VACACIONES", `Del ${fechaDesde} al ${fechaHasta}`)} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black">ENVIAR</button>
          </div>}
          {subPantalla === 'medico' && <div className="space-y-4 text-center">
            ...
            {!imgCertificado ? <button onClick={() => fileRef.current?.click()} className="w-full py-12 border-4 border-dashed rounded-2xl text-slate-400 font-black">CAPTURAR FOTO</button> : <img src={imgCertificado} className="w-full rounded-2xl" />}
            <button disabled={!imgCertificado} onClick={() => enviar("MEDICO", imgCertificado)} className="w-full bg-emerald-600 text-white py-4 rounded-2xl font-black">ENVIAR</button>
          </div>}
          {subPantalla === 'reclamo' && <div className="space-y-4">
            <textarea value={textoMensaje} onChange={(e) => setTextoMensaje(e.target.value)} className="w-full h-32 p-4 bg-slate-50 border rounded-2xl font-bold" placeholder="Mensaje..." />
            <button onClick={() => enviar("SUGERENCIA", textoMensaje)} className="w-full bg-rose-600 text-white py-4 rounded-2xl font-black">ENVIAR</button>
          </div>}
        </div>
      )}
    </div>
...
function GerenciaPage() {
  ...
  const validacionPorTanque = useMemo(() => {
    ...
    Object.keys(asignaciones).forEach(tId => {
      ...
      const actual = tankReadings?.[tId]?.liters || 0;
      const libre = config ? config.maxLiters - actual : 0;
  ...
  const totalCostoPedido = useMemo(() => {
    return camionState.reduce((acc, cis, idx) => {
      ...
      return acc + (CAMIONES_MEDIDAS[tipoCamion][idx] * (fuelPrices[cis.fuel] || 0));
    }, 0);
  }, [camionState, tipoCamion, fuelPrices]);

  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans relative">
      {viewImg && <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4" onClick={() => setViewImg(null)}><img src={viewImg} className="max-w-full max-h-full" /></div>}
      <aside className="w-full md:w-64 bg-slate-900 text-white p-6 flex flex-col shadow-2xl z-20">
        ...
        <nav className="space-y-1 flex-1">
          {[{ id: 'pedido', label: 'Carga Camión', icon: <Truck className="w-4"/> }, { id: 'tanques', label: 'Stock Online', icon: <Database className="w-4"/> }, { id: 'rrhh', label: 'Buzón RRHH', icon: <MessageSquare className="w-4"/> }].map(item => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`w-full flex items-center gap-3 p-4 rounded-xl text-[10px] font-black uppercase transition-all ${activeMenu === item.id ? 'bg-[#E20074] text-white' : 'text-slate-500 hover:bg-white/5'}`}>{item.icon} {item.label}</button>
          ))}
        </nav>
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            <div className="xl:col-span-2 space-y-6">
              <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border-t-8 border-emerald-500">
                <h3 className="font-black uppercase italic text-gray-800 mb-4 flex items-center gap-2 text-xs"><CircleDollarSign className="w-4 text-emerald-500"/> Costos de Reposición ($)</h3>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {Object.keys(fuelPrices).map(f => (
                    <div key={f} className="bg-slate-50 p-3 rounded-xl border"><p className="text-[8px] font-black text-slate-400 uppercase italic mb-1">{f}</p><input type="number" value={fuelPrices[f]} onChange={(e) => setFuelPrices({...fuelPrices, [f]: parseFloat(e.target.value)})} className="w-full bg-transparent font-black text-sm outline-none text-emerald-600" /></div>
                  ))}
                </div>
              </div>
              <div className="bg-white p-8 rounded-[3rem] shadow-sm border-t-8 border-slate-900">
                <div className="flex justify-between items-center mb-8">
                  <h3 className="font-black uppercase italic text-gray-800">Cisternas</h3>
                ...
                <div className="space-y-3">
                  {CAMIONES_MEDIDAS[tipoCamion].map((cap, idx) => (
                    <div key={idx} className={`p-4 rounded-2xl border-2 flex flex-col sm:flex-row items-center gap-3 transition-all ${camionState[idx].fuel !== 'vacio' ? 'border-[#E20074] bg-pink-50/20' : 'bg-slate-50'}`}>
                      <div className="w-24"><p className="text-[8px] font-black text-slate-400 uppercase leading-none">Cisterna {idx+1}</p><p className="text-sm font-black text-slate-800 italic">{cap.toLocaleString()} L</p></div>
                      <select value={camionState[idx].fuel} onChange={(e) => { const nc = [...camionState]; nc[idx] = { fuel: e.target.value, tankId: '' }; setCamionState(nc); }} className="flex-1 p-3 rounded-xl font-black text-[10px] uppercase border bg-white">
                        <option value="vacio">PRODUCTO</option><option value="super">SÚPER</option><option value="quantium_nafta">QUANTIUM N</option><option value="x10">DIESEL X10</option><option value="quantium_diesel">QUANTIUM D</option>
                      </select>
                      <select disabled={camionState[idx].fuel === 'vacio'} value={camionState[idx].tankId} onChange={(e) => { const nc = [...camionState]; nc[idx].tankId = e.target.value; setCamionState(nc); }} className="flex-1 p-3 rounded-xl font-black text-[10px] uppercase border bg-white disabled:opacity-30">
            ...
            <div className="space-y-6">
              <div className="bg-slate-900 p-8 rounded-[3rem] text-white shadow-2xl sticky top-4">
                ...
                {validacionPorTanque.length > 0 ? validacionPorTanque.map((al, i) => (
                  <div key={i} className="p-4 rounded-2xl bg-red-500/20 border border-red-500 mb-2 animate-pulse"><p className="text-[10px] font-black uppercase text-red-500">EXCEDIDO EN {al.name}</p><p className="text-lg font-black">+{al.sobra.toLocaleString()} L</p></div>
                )) : <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-center text-emerald-400 font-black text-[10px] uppercase italic">Pedido Seguro</div>}
                <div className="mt-8 pt-8 border-t border-white/10">
                   <p className="text-[9px] font-black text-[#E20074] uppercase italic">Costo Total</p>
                   <p className="text-4xl font-black italic tracking-tighter mb-6">$ {totalCostoPedido.toLocaleString()}</p>
                   <button onClick={() => window.print()} disabled={totalCostoPedido === 0 || validacionPorTanque.length > 0} className="w-full bg-[#E20074] py-4 rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2"><Printer className="w-4"/> Imprimir Nota</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {activeMenu === 'tanques' && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in">
            {TANKS_CONFIG.map(t => {
              ...
              const critico = liters < 2500;
              return (
                <div key={t.id} className={`bg-white p-6 rounded-[2.5rem] shadow-sm text-center border relative ${critico ? 'ring-4 ring-red-500/20' : ''}`}>
                  {critico && <div className="absolute top-0 left-0 right-0 bg-red-600 text-white text-[7px] font-black py-1">CRÍTICO</div>}
                  <div className="w-16 bg-slate-50 h-40 rounded-2xl relative overflow-hidden flex items-end border mx-auto mb-4"><div className={`w-full ${critico ? 'bg-red-600' : t.color} transition-all`} style={{ height: `${pct}%` }}></div><div className="absolute inset-0 flex items-center justify-center font-black text-[10px]">{pct}%</div></div>
                  <p className="text-[9px] font-black text-slate-400 uppercase italic">{t.name}</p><p className={`text-sm font-black ${critico ? 'text-red-600' : 'text-slate-800'}`}>{Math.round(liters).toLocaleString()} L</p>
                </div>
              );
            })}
          </div>
        ...
        {activeMenu === 'rrhh' && (
          <div className="space-y-4 animate-in slide-in-from-right-4">
             {solicitudes.map(sol => (
               <div key={sol.id} className="bg-white p-6 rounded-[2.5rem] shadow-sm border-l-8 border-slate-900 flex justify-between items-center">
                  <div><span className={`text-[8px] font-black px-2 py-0.5 rounded-full text-white ${sol.tipo === 'VACACIONES' ? 'bg-blue-500' : sol.tipo === 'MEDICO' ? 'bg-emerald-500' : 'bg-rose-500'}`}>{sol.tipo}</span><h3 className="font-black text-slate-800 text-sm mt-1 italic uppercase">{sol.empleado}</h3><p className="text-slate-500 text-xs italic">{sol.tipo === 'MEDICO' ? 'Certificado Médico' : `"${sol.contenido}"`}</p></div>
...
function PlayaPage() {
  ...
  if (!tankReadings) return <div className="min-h-screen flex items-center justify-center text-[#E20074] font-black uppercase">Cargando...</div>;
      ...
      <nav className="bg-[#E20074] p-4 text-white flex justify-between items-center shadow-lg sticky top-0 z-40">
        ...
        <div className="flex bg-black/20 p-1 rounded-xl gap-1">
          {['varillas', 'descarga', 'monitor', 'rrhh'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-lg text-[9px] font-black uppercase transition-all ${activeTab === tab ? 'bg-white text-[#E20074]' : 'text-white/60'}`}>{tab}</button>
          ))}
        </div>
      </nav>
      <main className="p-4 max-w-xl mx-auto w-full">
        {activeTab === 'varillas' && (
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-b-8 border-[#E20074]">
            <h2 className="text-lg font-black mb-6 italic uppercase flex items-center gap-3"><Ruler className="text-[#E20074]"/> Carga Varilla</h2>
            <div className="space-y-2">
              {TANKS_CONFIG.map(tank => (
                <div key={tank.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border">
                  <span className="text-[10px] font-black text-slate-400 uppercase italic">{tank.name}</span>
                  <div className="flex items-center gap-4">
                    <input type="number" value={tankReadings[tank.id]?.mm || ''} onChange={(e) => handleTankChange(tank.id, 'mm', e.target.value)} className="w-24 p-2 border-2 rounded-xl text-center font-black text-lg outline-none focus:border-[#E20074]" placeholder="0"/>
                    <div className="w-20 text-right font-black text-[#E20074] italic">{Math.round(tankReadings[tank.id]?.liters || 0).toLocaleString()}L</div>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={async () => { await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'estado_actual', 'varillas_oficiales_v4'), { readings: tankReadings }); alert("Guardado ✅"); }} className="w-full mt-8 bg-[#E20074] text-white py-5 rounded-2xl font-black uppercase italic shadow-xl">Guardar Mediciones</button>
          </div>
        )}
        {activeTab === 'descarga' && (
          <div className="bg-white p-6 rounded-[2.5rem] shadow-xl border-b-8 border-amber-500">
            ...
            <button onClick={async () => { await setDoc(doc(db, 'artifacts', appId, 'public', 'data', 'estado_actual', 'varillas_oficiales_v4'), { readings: tankReadings }); alert("Registrado ✅"); }} className="w-full mt-6 bg-amber-500 text-white py-4 rounded-xl font-black uppercase shadow-lg italic">Registrar Descarga</button>
          </div>
        )}
        {activeTab === 'monitor' && (
          <div className="bg-slate-900 p-6 rounded-[2.5rem] shadow-xl text-white text-center">
             <h2 className="text-sm font-black italic uppercase text-emerald-400 mb-6">Stock Online</h2>
              ...
              {TANKS_CONFIG.map(tank => {
                ...
                return (
                  <div key={tank.id} className="flex flex-col items-center gap-1 w-full">
                    ...
                    <div className="w-full max-w-[24px] bg-white/5 rounded-t relative h-32 flex items-end"><div className={`w-full ${tank.color} opacity-80`} style={{ height: `${percent}%` }}></div></div>
                  </div>
                );
              })}
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl w-full max-w-xs text-center border-t-8 border-[#E20074]">
            ...
            <form onSubmit={verificarPin}>
              ...
              <div className="flex gap-4"><button type="button" onClick={() => setTargetModulo(null)} className="flex-1 font-bold text-slate-400 uppercase text-[9px]">Cancelar</button><button type="submit" className="flex-1 bg-[#E20074] text-white py-4 rounded-2xl font-black uppercase text-[10px]">Entrar</button></div>
            </form>
          </div>
        </div>
      )}
      <header className="bg-white/10 backdrop-blur-md border-b border-white/20 py-8 px-10 flex items-center gap-4">
        <div className="h-14 w-14 bg-white p-2 rounded-2xl shadow-xl flex items-center justify-center"><img src="/logo.png" className="h-full object-contain" /></div>
        <div><h1 className="text-2xl font-black uppercase italic leading-none tracking-tighter">Gestión Operativa</h1><p className="text-white/80 text-[9px] font-bold uppercase tracking-widest mt-1 italic">AXION Crespo</p></div>
      </header>
      <main className="flex-grow flex items-center justify-center p-6 text-slate-800">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl w-full">
          {[ { id: 'playa', logo: "/playa.png", label: 'Playa / Tanques' }, { id: 'spot', logo: "/spot.png", label: 'Spot / Ventas' }, { id: 'gerencia', icon: '⚙️', label: 'Gerencia' } ].map(m => (
            <div key={m.id} onClick={() => setTargetModulo(m.id)} className="bg-white p-12 rounded-[3.5rem] shadow-2xl cursor-pointer hover:scale-105 transition-all text-center group border-4 border-transparent hover:border-white/50">
              {m.logo ? <img src={m.logo} className="h-20 mx-auto mb-6 object-contain" /> : <div className="h-20 w-20 mx-auto mb-6 bg-slate-50 rounded-full flex items-center justify-center border-4 border-slate-800 text-3xl font-black italic">⚙️</div>}
...
function SpotPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col font-sans">
      <nav className="bg-amber-500 p-4 text-white flex justify-between items-center shadow-lg"><button onClick={() => navigate('/')} className="font-black italic text-[10px] uppercase flex items-center gap-1"><X className="w-3 h-3"/> Volver</button></nav>
